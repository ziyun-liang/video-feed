...
function renderVideoFeed() {
  ...
  }).join('') + '<div class="ad-countdown"></div>';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Glow UX for ad lock ---
    function triggerAdCountdownGlow() {
        if (!adLock || currentIndex !== adIndex) return;
        const countdownDiv = document.querySelector('.ad-countdown');
        if (countdownDiv) {
            countdownDiv.classList.add('ad-countdown-glow');
            setTimeout(() => {
                countdownDiv.classList.remove('ad-countdown-glow');
            }, 700);
        }
    }
    ...
    document.body.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(diff) < 10) {
            // Tap: pause/play
            const container = document.elementFromPoint(e.changedTouches[0].clientX, e.changedTouches[0].clientY)?.closest('.video-container');
            if (container) handleTapPause({ ...e, currentTarget: container });
            gestureHandled = true;
            return;
    ...
    function lockAdSwipe() {
        ...
        const countdownDiv = document.querySelector('.ad-countdown');
        let seconds = 5;
        countdownDiv.textContent = `Skip ad in ${seconds}`;
        countdownDiv.classList.add('ad-countdown-active');
        countdownDiv.classList.remove('hide', 'skip');
        ...
        adCountdownTimer = setInterval(() => {
            ...
            if (seconds > 0) {
                countdownDiv.textContent = `Skip ad in ${seconds}`;
                countdownDiv.classList.remove('skip');
            } else {
                countdownDiv.classList.add('hide');
                setTimeout(() => {
                    countdownDiv.textContent = 'Swipe to continue \u2191';
                    countdownDiv.classList.remove('hide');
                    countdownDiv.classList.add('skip');
                }, 300);
                adLock = false;
                clearInterval(adCountdownTimer);
                setTimeout(() => {
                    countdownDiv.classList.add('hide');
                    countdownDiv.classList.remove('skip', 'ad-countdown-active');
                    countdownDiv.textContent = '';
                    setTimeout(() => {
                        countdownDiv.style.display = 'none';
                    }, 300);
                }, 4000); // Show skip message for 4s (was 2s)
            }
        }, 1000);
        // Add click handler for skip state
        countdownDiv.onclick = function() {
            if (countdownDiv.classList.contains('skip')) {
                goToIndex(adIndex + 1);
            }
        };
    }
    function unlockAdSwipe() {
        adLock = false;
        const countdownDiv = document.querySelector('.ad-countdown');
        countdownDiv.classList.remove('ad-countdown-active', 'skip', 'hide');
        countdownDiv.textContent = '';
        countdownDiv.style.display = 'none';
        if (adCountdownTimer) clearInterval(adCountdownTimer);
    }
    ...
    // --- Tap-to-pause/resume logic (from v2/v3, with countdown resume for ad) ---
    ...
    function handleTapPause(e) {
        ...
        if (!isPausedByTap) {
            ...
            if (currentIndex === adIndex && adLock) {
                if (adCountdownTimer) {
                    // Save remaining seconds from countdownDiv text
                    const countdownDiv = adContainer.querySelector('.ad-countdown');
                    const match = countdownDiv.textContent.match(/(\d+)/);
                    if (match) {
